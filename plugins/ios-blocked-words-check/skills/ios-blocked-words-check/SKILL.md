...
3. **主 agent 使用 AskUserQuestion 给用户选项**：
   - 在 `question` 字段中嵌入 **ASCII 美化表格**，展示所有违规明细，让用户在做决策时一目了然
   - 表格使用 box-drawing 字符绘制，格式如下：
     ```
     检测到以下敏感词违规：
     ...
     │ 敏感词   │ 文件                │ 行号 │ 匹配文本     │
     ├──────────┼─────────────────────┼──────┼──────────────┤
     │ pay      │ SendGift.m          │ L42  │ payCoins     │
     │ money    │ SendGift.m          │ L88  │ moneyAmount  │
     │ casino   │ GameVC.swift        │ L15  │ casinoView   │
     └──────────┴─────────────────────┴──────┴──────────────┘
