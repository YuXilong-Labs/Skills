...
3. **主 agent 将汇总结果以 Markdown 表格格式输出给用户**：
   - **直接输出 Markdown 表格**（`| col | col |` 格式），不要包裹在代码块内
   - Claude Code 终端会自动将 Markdown 表格渲染为带边框的等宽对齐表格
   - 渲染规则：
     - 保持脚本输出的管道表格结构，原样输出即可
     - 多行条目（同一敏感词多个 proto 溯源）的重复列（敏感词/命中数/文件数）置空
     - proto 字段定义列保留反引号包裹代码内容
   - 输出示例：
     ```
     敏感词检查汇总：3 个敏感词，共 14 处命中（已忽略注释）
     ...
     | 敏感词 | 命中数 | 涉及文件数 | 源 proto 文件 | proto 行号 | 字段定义 |
     |--------|--------|-----------|--------------|-----------|---------|
     | pay    | 6      | 2         | biz/xxx.proto | L26 | `int64 pay_coins = 1;` |
     |        |        |           | biz/xxx.proto | L54 | `int64 pay_type = 7;` |
     | win    | 4      | 1         | biz/yyy.proto | L30 | `string win_sticker = 16;` |

     共 3 个敏感词，14 处命中。
     ```

4. **主 agent 使用 AskUserQuestion 给用户选项**：
   - 在 `question` 字段中说明检测到敏感词违规，并提示用户查看上方表格
   - 选项：
