...
每个 Agent 的 prompt 必须包含完整的审查规范、维度、分级、输出格式（不要省略或引用外部文件，Agent 没有 Skill 上下文）。

> 完整 Agent Prompt 模板见 [agent-prompt-template.md](references/agent-prompt-template.md)
> **构造 prompt 时，必须将模板内容完整嵌入到 Agent 调用中，不可引用文件路径。**
