...
| `wk-xcodebuild` | `/wk-xcodebuild` | **xcodebuild 智能包装** — 自动选 USB 真机目标（无则回退 Mac），rtk 风格精简 build/test 输出（实测省 ~99.7% token）；test 后追加 xcresult 权威摘要（XCTest/Swift Testing 统一计数），`xcb result`/`xcb cov` 摘要测试结果与覆盖率，`xcb pod` 精简 pod install/update/lint 输出 |
...
| `wk-xcodebuild`（内置） | `PreToolUse` | 执行裸 `xcodebuild`、`swift build`/`swift test`、`pod install/update/lint` 或 `xcresulttool get test-results` 前 | 静默改写为包装器（`allow`+`updatedInput`）；`WK_XCB_BYPASS=1` 可逃生 |
